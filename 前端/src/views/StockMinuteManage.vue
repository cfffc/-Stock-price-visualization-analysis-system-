<template>
  <div style="display: flex">
    <el-upload :auto-upload="false" :limit="100" :on-change="add" :show-file-list="false" accept=".txt"
               action="#">
      <el-button style="flex-basis: fit-content; margin-right: 1rem" type="danger">添加分钟级股票数据</el-button>
    </el-upload>
...
export default {
  setup() {
    ...
    onBeforeMount(() => {
      initData();
      document.title = "分钟级股票数据管理";
    });

    function initData() {
      postRequest("/stockMinuteName/list", {
    ...
    function add(file) {
      ...
      postRequest("/stockMinuteData/add", formData).then((res) => {
        if (res.data.code === 200) {
    ...
    function exportData(row) {
      getRequest("/stockMinuteName/exportData?stockId=" + row.stockId).then((res) => {
    ...
    function deleteStock(row) {
      deleteRequest("/stockMinuteName/deleteStock?stockId=" + row.stockId).then((res) => {
        if (res.data.code === 200) {
          message(res.data.msg, "success");
          initData();
        } else if (res.data.code === 500) {
